...
class Mpb_Meta_Box_Sanitize {

	/**
	 * A MPB field object
	 * @var Mpb_Meta_Box_field object
	 */
	public $field;
	 ...
	 * @param object $field A MPB field object
	...
	public function default_sanitization( $value ) {
		...
		$updated = apply_filters( 'mpb_validate_'. $this->field->type(), null, $value, $this->object_id, $this->field->args(), $this );
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = Mpb_Meta_Box::timezone_string();

		$offset = Mpb_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new Mpb_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = Mpb_Meta_Box::image_id_from_url( $value );
		}
